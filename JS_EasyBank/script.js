...
const calcDisplaySummary = function (accountTemp) {
  const incomes = accountTemp.movements
    .filter(mov => mov > 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumIn.textContent = `${incomes} €`;
  const outcomes = accountTemp.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = `${Math.abs(outcomes)} €`;
  const Interest = accountTemp.movements
    .filter(mov => mov > 0)
    .map(mov => (mov * accountTemp.interestRate) / 100)
    .filter(int => int > 1)
    .reduce((acc, mov) => acc + mov, 0);
